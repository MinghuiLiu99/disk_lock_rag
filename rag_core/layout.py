...
# (?!条) 用来排除说明正文里的**引用式开头**——"6.2.3条给出此条规定…"，
# 那不是新的一条说明，而是上一段话的延续（DB11 实测出现，导致同一说明重复两次）。
NOTES_CLAUSE_RE = re.compile(r"^(\d+\.\d+\.\d+)(?!\d)(?!条)")
...
# 目录行：标题 + 一长串引导符 + 页码。目录页里的条目长得和正文标题一模一样
# （"附录A 典型产品构配件种类及规格 ......... 37"），不排除会被当成真标题，
# 于是同一份文档里出现两个"附录A"。JGJ/T 231 的 PDF 没有正文目录，
# 这个坑是在 DB11/T 2100 上才暴露的。
# 引导符有三种：中文目录用点线 "...."、英文目录用省略号 "…………"（U+2026）、
# 也有用间隔号 "····" 的。少写一种就会漏掉一整页目录。
